...
// Actions
import { updateGame } from "../contexts/UserActions";
...
export const EditGame = () => {
  ...
  const game = state.profile.gameLibrary.find((item) => {
    return item._id === gameId;
  });

  const [name, setName] = useState({
    field: game.name ? game.name : "",
    feedback: game.name ? "is-valid" : "",
    error: game.name ? false : true,
  });

  const [system, setSystem] = useState({
    field: game.system ? game.system : "",
    feedback: game.system ? "is-valid" : "",
    error: game.system ? false : true,
  });

  const [status, setStatus] = useState({
    field: game.status ? game.status : "",
    feedback: game.status ? "is-valid" : "",
    error: game.status ? false : true,
  });

  const [ownership, setOwnership] = useState({
    field: game.ownership ? game.ownership : "",
    disable:
      ...
      game.status === "Backlog"
        ? false
        : true,
    feedback: game.ownership ? "is-valid" : "",
    error: game.ownership ? false : true,
  });

  const [hours, setHours] = useState({
    field: game.hours ? game.hours : "",
    disable:
      game.status === "Unfinished" ||
      game.status === "Ongoing" ||
      game.status === "Beaten" ||
      game.status === "Completed"
        ? false
        : true,
    feedback: game.hours ? "is-valid" : "",
    error: game.hours ? false : true,
  });

  const [rating, setRating] = useState({
    field: game.rating ? game.rating : "",
    disable:
      game.status === "Unfinished" ||
      game.status === "Ongoing" ||
      game.status === "Beaten" ||
      game.status === "Completed"
        ? false
        : true,
    feedback: game.rating ? "is-valid" : "",
    error: game.rating ? false : true,
  });

  const [review, setReview] = useState({
    field: game.review ? game.review : "", // This is optional
    disable:
      game.status === "Unfinished" ||
      game.status === "Ongoing" ||
      game.status === "Beaten" ||
  ...
  const [comments, setComments] = useState({
    field: game.comments ? game.comments : "", // This is optional
  });
